...
function Statusinfo($statusinfo) {
  ...
  that.setPhisicStat = function(newPhisicStat) {
    if (ps !== newPhisicStat.ps_smooth ||
        dutyRatio !== newPhisicStat.dutyRatio_smooth) {
      ps = newPhisicStat.ps_smooth;
      dutyRatio = newPhisicStat.dutyRatio_smooth;
      ...
      }
      {
        ...
        } else if (dutyRatio < 0.3) {
          humanizedStringDuty = 'Ок';
        } else if (dutyRatio < 0.45) {
          humanizedStringDuty = 'Ок, если это ноутбук или смартфон, на этом уровне производительности уже может греться и кушать батарею. В этом случае можно попробовать уменьшить настройки графики.';
        } else if (dutyRatio < 0.6) {
          humanizedStringDuty = 'Физический-цикл потребляет слишком много ресурсов. Попробуйте уменьшить настройки графики.';
          errorLevelDuty = 'yellow';
        } else {
          humanizedStringDuty = 'Физический-цикл потребляет критически много ресурсов! Попробуйте уменьшить настройки графики.';
          errorLevelDuty = 'red';
        }
      }
